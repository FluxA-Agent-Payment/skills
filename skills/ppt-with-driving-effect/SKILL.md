---
name: PPT Generator Pro with Driving Effect
...
1. **Send Generation Request** via Image Generation Tool (use `/gen-image-pro` for best quality)
...
1. **Submit Generation Task** via Video Generation Tool (Kling `/image-to-video`)
...
2. **Poll Task Status** via `/get-video` (free endpoint) until video is ready

3. **Download and Save Videos** to `output/ppt_TIMESTAMP/videos/`
