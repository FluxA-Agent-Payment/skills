# Integrate Kling Video Generation API

**API Endpoint:** `https://proxy-monetize.fluxapay.xyz/api/kling-video-generation/7ded04c9`
...
|--------|------|-------------|-------|
| GET | `/get-video` | Get video generation task status and result | Free |
| POST | `/image-to-video` | using image url to generate video with kling v2.6 | $0.20 / 0.20 cr |
...
```bash
curl https://proxy-monetize.fluxapay.xyz/api/kling-video-generation/7ded04c9/<endpoint>
```
...
```bash
curl -H "X-Payment: <xPaymentB64>" https://proxy-monetize.fluxapay.xyz/api/kling-video-generation/7ded04c9/<endpoint>
```
